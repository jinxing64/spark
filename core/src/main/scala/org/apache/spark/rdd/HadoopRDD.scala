  ...
  override def getPartitions: Array[Partition] = {
    ...
    if (inputSplits.size < maxPartitions) {
      ...
      for (i <- 0 until inputSplits.size) {
        array(i) = new HadoopPartition(id, i, Array(inputSplits(i)))
      }
      array
    } else {
      val array = new Array[Partition](maxPartitions)
      val base = inputSplits.size / maxPartitions
      var idx = 0
      for (i <- 0 until inputSplits.size % maxPartitions) {
        val piece = new Array[InputSplit](base + 1)
        for (j <- idx until base + 1) {
          piece(j) = inputSplits(j)
        }
        array(i) = new HadoopPartition(id, i, piece)
        idx += base + 1
      }
      for (i <- inputSplits.size % maxPartitions until maxPartitions) {
        val piece = new Array[InputSplit](base)
        for (j <- idx until base) {
          piece(j) = inputSplits(j)
        }
        array(i) = new HadoopPartition(id, i, piece)
        idx += base
  ...
  override def getPreferredLocations(part: Partition): Seq[String] = {
    ...
    val locs = hsplits.head match {
      case _: InputSplitWithLocationInfo =>
        ...
        HadoopRDD.convertSplitLocationInfo(lsplits.flatMap(_.getLocationInfo))
      case _ => None
    }
